...
async function handleProxy(request: NextRequest, context: { params: Promise<{ path: string[] }> }) {
  ...
  // Costruisce l'URL di destinazione sul server API Hono (es. http://localhost:3001/geolocation/...)
  const targetUrl = new URL(`/geolocation/${pathString}`, API_BASE_URL);
  ...
  try {
    ...
    console.error(`[SSO Geolocation Proxy] Error proxying to ${targetUrl}:`, err instanceof Error ? err.message : err);
