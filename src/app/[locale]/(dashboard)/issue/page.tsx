...
export default function SupportTicketsPage() {
  ...
  const handleCreateTicket = (e: React.FormEvent) => {
    ...
    setNewTitle("");
    setNewMessage("");
    showToast("Ticket di supporto creato con successo.", "success");
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in font-sans">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-black tracking-tight uppercase text-slate-900 dark:text-white">Segnalazioni & Assistenza</h2>
          <p className="text-sm text-slate-500 dark:text-gray-400">Apri un ticket di supporto per problemi commerciali, amministrativi o guasti hardware.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-4">
          <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-2">I Miei Ticket</h3>
          {tickets.length === 0 ? (
            <Card className="border border-slate-200 dark:border-white/10 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl rounded-3xl p-12 text-center shadow-xl">
              <HelpCircle className="w-12 h-12 text-slate-300 dark:text-slate-700 mx-auto mb-4" />
              <h4 className="text-sm font-bold text-slate-950 dark:text-white">Nessuna segnalazione attiva</h4>
              <p className="text-xs text-slate-400 font-semibold max-w-sm mx-auto mt-1">Usa il form a fianco per aprire una segnalazione al team di supporto.</p>
            </Card>
          ) : (
            tickets.map((ticket) => (
              <Card key={ticket.id} className="border border-slate-200 dark:border-white/10 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl rounded-3xl p-6 shadow-xl space-y-4 hover:scale-[1.005] transition-all">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex items-center gap-1.5 font-bold text-[10px] uppercase text-slate-600 dark:text-slate-300">
                    {getStatusIcon(ticket.status)} {ticket.status === "resolved" ? "Risolto" : (ticket.status === "open" ? "Aperto" : "In corso")}
                  </div>
                </div>
        ...
        <div>
          <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-2">Nuova Segnalazione</h3>
          <Card className="border border-slate-200 dark:border-white/10 bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl rounded-3xl p-6 shadow-xl">
            <form onSubmit={handleCreateTicket} className="space-y-4">
              <TextField isRequired className="flex flex-col gap-1.5 w-full">
                <Label className="text-xs font-bold text-slate-700 dark:text-gray-300 block mb-0.5">Oggetto</Label>
                <Input
              ...
              <div className="space-y-1">
                <label className="text-[10px] text-slate-400 font-bold uppercase block">Categoria</label>
                <select
                  value={newCategory}
                ...
                >
                  <option value="Fatturazione" className="bg-white dark:bg-slate-900">Fatturazione & Pagamenti</option>
                  <option value="Dispositivi" className="bg-white dark:bg-slate-900">Guasti Hardware</option>
                  <option value="SSO" className="bg-white dark:bg-slate-900">Accesso & Credenziali</option>
                  <option value="Altro" className="bg-white dark:bg-slate-900">Altre Richieste</option>
                </select>
              </div>
              <TextField isRequired className="flex flex-col gap-1.5 w-full">
                <Label className="text-xs font-bold text-slate-700 dark:text-gray-300 block mb-0.5">Messaggio</Label>
                <TextArea
              ...
              >
                Invia Segnalazione <Send className="w-3.5 h-3.5" />
              </Button>
            </form>
          </Card>
        </div>
