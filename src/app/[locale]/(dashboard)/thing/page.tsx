...
export default function ThingManagementPage() {
  ...
  const loadThings = useCallback(async (orgId: string) => {
    ...
    try {
      ...
      showToast(tRef.current("thing.toastLoadErr"), "error");
    } finally {
  ...
  const handleRemoveThing = async (thingId: string) => {
    try {
      ...
      showToast(t("thing.toastRemoveQueued"), "success");
      setThings(prev => prev.map(item => item.thingId === thingId ? { ...item, status: "deleting" } : item));
      ...
      console.error(err);
      showToast(t("thing.toastRemoveErr"), "error");
    }
  };

  const copyToken = () => {
    void navigator.clipboard.writeText(generatedThingToken);
    showToast(t("thing.toastTokenCopied"), "success");
  };
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {canCreate && (
          <Card className="border border-slate-200 dark:border-white/10 bg-white/70 dark:bg-slate-900/40 backdrop-blur-xl shadow-xl rounded-3xl p-1 lg:col-span-1">
            <CardContent className="p-2 space-y-4">
              <div>
                <h3 className="text-md font-extrabold text-slate-900 dark:text-white">{t("thing.registerTitle")}</h3>
                <p className="text-slate-500 dark:text-gray-400 text-[10px] mt-0.5">{t("thing.registerDesc")}</p>
              </div>
              ...
              <Form
                ...
                submitLabel={t("thing.registerBtn")}
              />
            </CardContent>
          </Card>
        )}
        ...
        {/* Elenco Cose */}
        <Card className={`border border-slate-200 dark:border-white/10 bg-white/70 dark:bg-slate-900/40 backdrop-blur-xl shadow-xl rounded-3xl p-1 ${canCreate ? "lg:col-span-2" : "lg:col-span-3"}`}>
          <CardContent className="p-2 space-y-4">
            <div>
              <h3 className="text-md font-extrabold text-slate-900 dark:text-white">{t("thing.listTitle")}</h3>
              <p className="text-slate-500 dark:text-gray-400 text-[10px] mt-0.5">{t("thing.listDesc")}</p>
            </div>
            ...
            {loadingData ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-start border-collapse">
                  <thead>
                    <tr>
                      <th className="bg-slate-100 dark:bg-white/5 font-bold text-xs px-4 py-3 text-slate-500 dark:text-slate-400 first:rounded-l-xl last:rounded-r-xl">{t("thing.colName")}</th>
                      <th className="bg-slate-100 dark:bg-white/5 font-bold text-xs px-4 py-3 text-slate-500 dark:text-slate-400 first:rounded-l-xl last:rounded-r-xl">{t("thing.colType")}</th>
                      <th className="bg-slate-100 dark:bg-white/5 font-bold text-xs px-4 py-3 text-slate-500 dark:text-slate-400 first:rounded-l-xl last:rounded-r-xl">{t("thing.colApp")}</th>
                      <th className="bg-slate-100 dark:bg-white/5 font-bold text-xs px-4 py-3 text-slate-500 dark:text-slate-400 first:rounded-l-xl last:rounded-r-xl">{t("thing.colStatus")}</th>
                      <th className="bg-slate-100 dark:bg-white/5 font-bold text-xs px-4 py-3 text-slate-500 dark:text-slate-400 first:rounded-l-xl last:rounded-r-xl text-end">{t("thing.colRemove")}</th>
                    </tr>
                  </thead>
