 ...
 *
 * Due varianti:
 * - `split` (default): griglia 40%–60% su lg (grid-cols-5: 2+3) — colonna form
 *   (landmark <main>) a inizio riga e pannello contenuti/marketing (landmark
 *   <aside>) nascosto sotto lg (su mobile resta solo il form, stack verticale).
...
const ROOT_CLASSES = {
  split:
    "min-h-screen w-full bg-slate-50 dark:bg-slate-950 flex flex-col lg:grid lg:grid-cols-5 relative overflow-hidden font-sans transition-colors duration-500",
  centered:
...
const FORM_CLASSES = {
  split: "flex-1 lg:col-span-2 flex flex-col justify-center items-center p-6 sm:p-12 relative z-10 w-full",
...
const AuthLayoutAside: React.FC<AuthAsideProps> = ({ children, className = "" }) => (
  <aside
    className={`hidden lg:flex lg:col-span-3 relative flex-col justify-between items-center p-16 overflow-hidden bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50 dark:from-slate-900 dark:via-slate-950 dark:to-black border-s border-slate-200 dark:border-white/5 transition-colors duration-500 ${className}`}
  >
    {children}
  </aside>
);

AuthLayoutAside.displayName = "AuthLayout.Aside";

// Re-export nominali + compound — pattern unico del framework: Object.assign.
// (Nominali <File><Member>: `AuthForm` è il componente form di login in ./AuthForm.)
export { AuthLayoutForm, AuthLayoutAside };
